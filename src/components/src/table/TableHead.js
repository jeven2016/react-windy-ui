...
const TableHead = React.forwardRef((props, ref) => {
  ...
  const getCheckTh = useCallback((otherProps) => {
    if (checkable) {
      return <th className="cell-check" {...otherProps}>
        {canCheckAll &&
        (isCheckbox ?
  ...
  const createTh = useCallback((cell, index, cellProps) => {
    let isDesc, isAsc;
      ...
      const tr = <tr key={`tr-${rowId}`}>
        {rowId === 1 && getCheckTh(defaultRowSpan)}
        {
          node.nodes.map((n, i) => {
            ...
            if (n.isTitle) {
              otherProps.extraClassName = `${otherProps.extraClassName ||
              ''} title`;
            }
        ...
        {
          rowId === 1 && scrollY &&
          <th className='scroll-th' {...defaultRowSpan}
