...
const HeadCell = React.forwardRef((props, ref) => {
      ...
      return cell.elements.map((elem, index) => {
        ...
        return (
          <Popover
            autoWidth
            hasArrow={false}
            key={`popover-${elem.key}`}
            offset={5}
            onChange={(popActive) => !popActive && setActiveFilter(null)}
            body={<ElemBody tableProps={props} />}
            position="bottomRight"
          >
            <div className={elemClsName} onClick={(e) => setActiveFilter({ key: elemKey })}>
              {elem.head}
            </div>
          </Popover>
        );
  ...
  return (
    <th className={thClsName} ref={multiRef} style={style} key={thKey} {...otherProps}>
          ...
          >
            ...
            {cell.sortable && (
              <div className="sort-column">
                <span className={`arrow-icon up-arrow ${isAsc ? 'text color-blue' : null}`} />
                <span className={`arrow-icon down-arrow ${isDesc ? 'text color-blue' : null}`} />
              </div>
            )}
          </div>
          {headElements()}
