...
const PureInput = React.forwardRef((props, ref) => {
  ...
  let clsName = clsx(extraClassName, className, inputSize, getErrorClsName(errorType), {
    'read-only': readOnly,
...
const IconInput = React.forwardRef((props, ref) => {
  ...
  const restIcons = useCallback(() => {
    return React.Children.map(rightIcons, (node) => (
      <span
  ...
  return (
    <span className={clsName} {...rootProps} ref={rootRef}>
      {leftIcon && restIcons()}
      <PureInput
        ref={multiInputRef}
        canFocus={false}
...
const Password = React.forwardRef((props, ref) => {
  ...
  validate(toggleIcons.length === 2, 'There should be two icons for selection.');
