...
const DateTimePanel = React.forwardRef((props, ref) => {
  const { datePanel, ...rest } = props;

  const { date, tempDate, onChange, locale, tryClosePopup, hasFooter } = useContext(DateContext);
  ...
  return (
    <div className="datetime-picker card" ref={ref} style={{ width: '35rem' }} {...rest}>
      <div className="picker-panels">
        <div className="data-panel">{datePanel}</div>
        <div className="time-panel">{timePanel}</div>
      </div>
