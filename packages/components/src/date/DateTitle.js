...
export default function DateTitle(props) {
  const { setPanelType, ...otherProps } = props;
  ...
  let txtContent = useMemo(
    () => (
      <span>
        {isDateType && currentDayOfWeek + ','} {currentMonth} {isDateType && currDate.date()}
      </span>
    ),
    [currDate, currentDayOfWeek, currentMonth, isDateType]
  );
  ...
  return (
    <Card.Header extraClassName="date-picker-header">
      <div className="date-picker-title" {...otherProps}>
        <div>
          <span className="year-info" onClick={() => setPanelType(PickerPanel.year)}>
            {currDate.year()}
          </span>
        </div>
        {!isYearType && <div className="detail-info">{txtContent}</div>}
      </div>
    </Card.Header>
  );
}
