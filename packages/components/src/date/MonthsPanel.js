...
const MonthsPanel = React.forwardRef((props, ref) => {
  ...
  const monthCnt = useMemo(() => {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      const col = (
        <Col justify="center" key={`col-${i}`}>
          <Button
            inverted
            initOutlineColor
            type={isGrayBtn(i) ? 'secondary' : 'primary'}
          ...
          <div className="dp-body">
            <div className="date-picker-info">
              ...
              <span className="content">
                <span className="year-range">{usePanelHead(startYear, changeYear)}</span>
              </span>
              <span className="next" />
            </div>
            {usePanel(monthCnt)}
