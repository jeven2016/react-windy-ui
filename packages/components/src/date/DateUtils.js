...
export const usePanelHead = (content, onClick, isNode = false) => {
  return isNode ? (
    ...
    <Button extraClassName="range-btn" inverted type="primary" onClick={onClick}>
      {content}️
    </Button>
  );
  /*return <Spring
...
const isActiveDay = (activeDate, displayDate, selectedDate) => {
  ...
  if (!activeDate.isSame(displayDate, 'month') || !activeDate.isSame(displayDate, 'year')) {
    return false;
  }
...
export const createDateColumns = ({ date, columnCount, tempDate, onChange, showPopup }) => {
...
export const useCloseButton = (autoClose, tryClosePopup, locale) => {
  return (
    !autoClose && (
      <Button type="primary" size="small" inverted onClick={tryClosePopup}>
        {locale.ok}
      </Button>
    )
  );
...
export const preYear = (year, rangeNumber = 100) =>
  year - rangeNumber <= 1000 ? 1000 : year - rangeNumber;
export const nextYear = (year, rangeNumber = 100) =>
  year + rangeNumber > 3000 ? 3000 : year + rangeNumber;

export const createTimeItems = ({ max, onClick }) => {
  return (
    <>
      {[...Array(max).keys()].map((i) => (
        <div className="tp-item" key={i} onClick={onClick.bind(null, i)}>
          {i < 10 ? '0' : ''}
          {i}
        </div>
      ))}
