...
const TimePicker = React.forwardRef((props, ref) => {
  ...
  const defaultDate = useMemo(
    () => convertDate(defaultTime, [format, 'H:M:S']),
    [format, defaultTime]
  );
  ...
  useEffect(() => {
    validate(
      nonNil(defaultDate),
      `the defaultValue '${defaultTime}' should be in valid date format.${format}`,
      isBlank(defaultTime)
    );

    validate(
      nonNil(realDate),
      `the value '${time}' should be in valid date format ${format}}`,
      isBlank(time)
    );
  ...
  return (
    ...
    >
      {inline ? (
        ...
        <Wrapper ctrlRef={ref} ref={wrapperRef} popupType={popupType} body={popupBody} {...rest} />
      )}
    </DateContext.Provider>
  );
});
