...
const Item = React.forwardRef((props, ref) => {
  ...
  const more = useMemo(
    () =>
      convertToArray(moreElements).map((elem, i) => React.cloneElement(elem, { key: `more-${i}` })),
    [moreElements]
  );
  ...
  const actionContent = useMemo(
    () =>
      actionArray.length > 0 && (
        <div className={listActionsCls}>
          {actionArray.map((act, i) => {
            return (
              <div className="list-action" key={'action-' + i}>
                {act}
              </div>
            );
          })}
...
const List = React.forwardRef((props, ref) => {
  const { className, extraClassName, header, footer, children, ...rest } = props;

  return (
    <div className="common-list" {...rest}>
