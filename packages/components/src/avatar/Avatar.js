...
const Avatar = React.forwardRef((props, ref) => {
  ...
  const avatarNodeProps = useMemo(
    () => (isNil(accessory) ? { ref: ref, ...rest } : rest),
    [accessory, ref, rest]
  );
  const avatarNode = (
    <div className={clsName} style={style} {...avatarNodeProps}>
  ...
  return isNil(accessory) ? (
    avatarNode
  ) : (
    <div className={rootCls} ref={ref}>
      {avatarNode}
      {realAccessory}
    </div>
  );
