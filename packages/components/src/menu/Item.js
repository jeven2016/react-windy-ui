...
const Item = React.forwardRef((props, ref) => {
  ...
  const clsName = clsx(extraClassName, className, {
    ...
    'right-bar': ctx.type === MenuType.primary && ctx.primaryBarPosition === 'right'
  });
  ...
  const content = useMemo(() => {
    ...
    return !show ? (
      ...
      <>
        ...
        <animated.div className="item-info" style={innerProps}>
          {children}
        </animated.div>
      </>
    );
  ...
  const paddingStyle = useMemo(
    () =>
      ctx.autoIndent
        ...
        : null,
    [ctx.autoIndent, ctx.indentUnit, ctx.indentation, ctx.initIndent, ctx.popupSubMenu, level]
  );
  ...
  if (!show && directChild) {
    return (
      <Tooltip body={children} position="right">
        {renderCnt}
      </Tooltip>
    );
  }
