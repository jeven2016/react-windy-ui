          ...
          >
            ...
            <Navbar.List>
              <Navbar.Item hasBar>
                ...
                <Link to={`/docs/${theme}`}>{intl.get('global.home.link.documentation')}</Link>
              </Navbar.Item>
              <Navbar.Item hasBar>
                <FontAwesomeIcon icon={faLaptopHouse} style={{ marginRight: '.5rem' }} />
                {intl.get('global.home.link.template')}
              </Navbar.Item>
              <Navbar.Item>
                <FontAwesomeIcon icon={faLightbulb} style={{ marginRight: '.5rem' }} />
                <Popover body={<Contributors />} activeBy="click" position="bottomRight">
                  <span style={{ color: '#fff' }}>{intl.get('global.home.link.contribute')}</span>
                </Popover>
              </Navbar.Item>
              <Navbar.Item>
                <FontAwesomeIcon icon={faSignLanguage} style={{ marginRight: '.5rem' }} />
                <Dropdown
                  activeBy="hover"
                  title={<span>{intl.get('global.home.link.language')}</span>}
                  onSelect={setLocale}
                >
                  <Dropdown.Menu>
                    {supportLocals.map((sl) => (
                      <Dropdown.Item
                        id={sl}
                        key={sl}
                        extraClassName={sl === locale ? 'doc selected-item' : null}
                      >
                        {intl.get(`locale.${sl}`)}
                      </Dropdown.Item>
                    ))}
              ...
              <Navbar.Item>
                <FontAwesomeIcon icon={faThList} style={{ marginRight: '.5rem' }} />
                <Dropdown
                  activeBy="hover"
                  title={<span>{intl.get('global.home.link.theme')}</span>}
