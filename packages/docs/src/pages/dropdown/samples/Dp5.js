...
const createDropdown = (text, position, activeBy) => {
  ...
  const title = (
    <Tooltip body={position} position={tooltipPosition}>
      <Button
        color="primary"
        outline
        initOutlineColor
  ...
  return (
    <Dropdown position={position} title={title} activeBy={activeBy}>
      <Dropdown.Menu type="primary">
        <Dropdown.Item onClick={() => console.log('click item1')}>Menu Item1</Dropdown.Item>
        <Dropdown.Item onClick={(e) => console.log('click item2')}>Menu Item2</Dropdown.Item>
        <Dropdown.Item onClick={(e) => console.log('click item3')}>Menu Item3</Dropdown.Item>
      </Dropdown.Menu>
    </Dropdown>
  );
};
