...
import { Button, Input, Notification, Toggle, Select, Tooltip } from 'react-windy-ui';
  ...
  return (
    <>
      <div className="doc doc-row space">
        ...
        <Tooltip body={<span>{`each message will be closed in ${duration}ms`}</span>}>
          <Input value={duration} onChange={(val) => changeDuration(val)} />
        </Tooltip>
      </div>
      <div className="doc doc-row">
        <Toggle
          active={hasCloseIcon}
          onChange={(val) => changeCloseIcon(val)}
      ...
      <Button onClick={() => Notification.mini('This is a message.')}>mini</Button>
    </>
  );
}
